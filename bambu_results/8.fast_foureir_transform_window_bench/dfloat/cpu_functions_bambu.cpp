...
static bench_t bambu_mod_pi(bench_t x) {
    bench_t two_pi = 6.28318530717958647692;
    bench_t pi     = 3.14159265358979323846;
    for (int i = 0; i < 20; i++) {
        if (x >  pi) x = x - two_pi;
        if (x < -pi) x = x + two_pi;
    }
...
// sin(x) via Taylor: x - x^3/3! + x^5/5! - ... (12 terms)
static bench_t bambu_sin(bench_t x) {
    x = bambu_mod_pi(x);
    bench_t term = x, result = x;
    for (int i = 1; i < 12; i++) {
        bench_t denom = (bench_t)((2*i) * (2*i + 1));
        term = -term * (x * x) / denom;
        result = result + term;
    }
    return result;
...
void aux_fft_function(bench_t* data, int64_t nn, int64_t start_pos){
    ...
    while (n>mmax) {
        istep = mmax<<1;
        //theta = -(2*M_PI/mmax);
	theta = -(2*3.14159265358979323846/mmax);
        //wtemp = sin(0.5*theta);
	wtemp = bambu_sin(0.5*theta);
        wpr = -2.0*wtemp*wtemp;
        //wpi = sin(theta);
	wpi = bambu_sin(theta);
        wr = 1.0;
