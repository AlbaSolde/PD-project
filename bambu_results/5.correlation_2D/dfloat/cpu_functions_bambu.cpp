...
// Modification --> sqrt approximation
static result_bench_t bambu_sqrt(result_bench_t x){
        if (x <= 0)
                return 0;

        result_bench_t guess = x;
        for (int i = 0; i < 20; i++){
                guess = (result_bench_t)0.5 * (guess + x / guess);
        }
        return guess;
}
...
void correlation_2D(const bench_t* A, const bench_t* B, result_bench_t* R ,const int size){
	...
	// Modification --> call the sqrt approximation
	result_bench_t denom = bambu_sqrt(acumulate_value_a_a * acumulate_value_b_b);
	if (denom == 0)
        	*R = 0;
	else
        	*R = (result_bench_t)(acumulate_value_a_b / denom);
