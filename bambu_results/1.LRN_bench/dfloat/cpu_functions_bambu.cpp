...
{
        // === BAMBU MODIFICATION ===
        // Bambu HLS only supports +, -, *, / for float 32-bit.
        // No sqrt, pow, exp or log functional units available.
        // Original formula: B = A / (K + ALPHA*A^2)^0.75
        // where K=2, ALPHA=1e-3, BETA=0.75
        //
        // x^0.75 approximated via 3 Newton-Raphson iterations for x^(3/4):
        // Using identity: x^0.75 = x / x^0.25
        // x^0.25 via two Newton-Raphson sqrt steps:
        //   Step 1: y = initial guess
        //   Step 2: y = 0.5*(y + x/y)  (converges to sqrt(x))
        //   Step 3: repeat on result    (converges to sqrt(sqrt(x)) = x^0.25)
        // Then x^0.75 = x * (1/x^0.25) ... but needs division.
        // Simpler: x^0.75 = x^0.5 * x^0.25, computed via NR.
        // === END MODIFICATION ===
        ...
        for (unsigned int i = 0; i < size; ++i)
        {
                ...
                {
                        float a = A[i*size+j];
                        float base = 2.0f + 10e-4f * a * a;

                        // Newton-Raphson sqrt(base): y -> 0.5*(y + base/y)
                        float y = base * 0.5f;           // initial guess
                        y = 0.5f * (y + base / y);       // iteration 1
                        y = 0.5f * (y + base / y);       // iteration 2
                        y = 0.5f * (y + base / y);       // iteration 3
                        // y ~ sqrt(base)

                        // Newton-Raphson sqrt(y) = base^0.25
                        float z = y * 0.5f;              // initial guess
                        z = 0.5f * (z + y / z);          // iteration 1
                        z = 0.5f * (z + y / z);          // iteration 2
                        z = 0.5f * (z + y / z);          // iteration 3
                        // z ~ base^0.25

                        // base^0.75 = base^0.5 * base^0.25 = y * z
                        float base075 = y * z;

                        B[i*size+j] = a / base075;
                }
        }
